...
BASE_FOLDER = os.path.join(CURRENT_DIR, "MS TEAM-ERA")
...
GARENA = "QkxBQ0tfQVBJcw=="  # BY MSTEAM in base64
    ...
    def save_rare_account(self, account_data, rarity_type, reason, rarity_score, is_ghost=False):
        try:
            ...
            rare_entry = {
                ...
                'region': "BY MSTEAM" if is_ghost else account_data.get('region', 'UNKNOWN'),
    ...
    def save_couples_account(self, account1, account2, reason, is_ghost=False):
        try:
            ...
            couples_entry = {
                ...
                'reason': reason,
                'region': "BY MSTEAM" if is_ghost else account1.get('region', 'UNKNOWN'),
    ...
    def save_normal_account(self, account_data, region, is_ghost=False):
        try:
            ...
            account_entry = {
                ...
                'account_id': account_data.get("account_id", "N/A"),
                'name': account_data["name"],
                'region': "BY MSTEAM" if is_ghost else region,
...
def generate_accounts():
    ...
    account_name = data.get('account_name', 'BY MSTEAM')
    password_prefix = data.get('password_prefix', 'BY MSTEAM')
